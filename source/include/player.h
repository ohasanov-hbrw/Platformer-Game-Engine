void MovePlayer(Tilemap* map ,playerStruct*Player, boxStruct *Box, int number, entityStruct* all)
{
	...
	for (int i = 0; i < number; i++) {
			...
			Rectangle entitybox = { Player->Position.x + Player->accel - 6 ,Player->Position.y + Player->yvel - 28,12,28 };
			Rectangle allbox = { all[i].Position.x,all[i].Position.y,all[i].width,all[i].height };
			...
			if (CheckCollisionRecs(entitybox, allbox)) {
				...
				MoveEntity(map, &all[i], number, all, i, false, -2, &Player);

				if (Player->accel < 0) {
					Player->acceldegisken = 0.5f;
				}
				else if (Player->accel > 0) {
					Player->acceldegisken = -0.5f;
				}
				else checkx = false;
				if (Player->yvel < 0) {
